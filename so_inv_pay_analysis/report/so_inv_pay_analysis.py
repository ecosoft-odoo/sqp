...
class so_inv_pay_report(osv.osv):

    _name = "so.inv.pay.report"
    ...
    _columns = {
        ...
        'is_international': fields.boolean('International', readonly=True),
        ...
        'percent_paid': fields.float('% Paid by Sales Order', readonly=True, group_operator="sum"),
    }
    _order = 'date desc'
    ...
    def init(self, cr):
        ...
        cr.execute("""CREATE or REPLACE VIEW so_inv_pay_report as (
                ...
                select *, (case when coalesce(amount_total, 0) = 0 then 0 else ((invoiced_amount / amount_total) * residual) end)::decimal(16,2) as unpaid_amount,
                    ...
                    from (
                        select ai.id as id,
                            ...
                            so.is_international,
                            (case when coalesce(so.amount_final, 0) = 0 then so.amount_untaxed else so.amount_final end) as order_amount,
                            ai.id as invoice_id,
